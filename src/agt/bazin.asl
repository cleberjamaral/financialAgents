...
+!reply(S,Q)[source(self)] : fundamentals::divYield(P) & fundamentals::divBrutaSobrePatrimonio(D) & P >= 6 & D < 1 <-
	.concat("Baseado no metodo de Bazin, ", S," tem Yield >=6% (",P,"%) e Div./ Patr <=1 (",D,"): COMPRAR", CCC)
	.send(Q,tell,recommend(S,comprar,CCC));
	.

+!reply(S,Q)[source(self)] : fundamentals::divYield(P) & fundamentals::divBrutaSobrePatrimonio(D) <-
